...
export default function App() {
  ...
  const handleRemoveProfileImage = (e: React.MouseEvent) => {
    ...
    addLog('🗑️ Profile photo removed.', 'info');
  };
        ...
        {activeTab === 'simulator' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col items-center">
              ...
              <div className="w-full max-w-[420px] flex items-center justify-between mb-3 px-3 py-2 rounded-2xl bg-slate-900/90 border border-slate-800 text-sm text-slate-300 shadow-md">
                <span className="text-xs text-slate-400 font-semibold">Active Screen:</span>
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => { setSimState('IDLE'); setCurrentAppTab('HOME'); }}
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                      simState === 'IDLE' && currentAppTab === 'HOME' ? 'bg-emerald-500 text-slate-950 shadow' : 'bg-slate-800/80 text-slate-300 hover:text-white'
                    }`}
                  >
                    Home
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                      simState === 'IDLE' && currentAppTab === 'FRIENDS' ? 'bg-emerald-500 text-slate-950 shadow' : 'bg-slate-800/80 text-slate-300 hover:text-white'
                    }`}
                  >
                    Friends
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                      simState === 'IDLE' && currentAppTab === 'SUBSCRIPTION' ? 'bg-emerald-500 text-slate-950 shadow' : 'bg-slate-800/80 text-slate-300 hover:text-white'
                    }`}
                  >
                    5-Mo Plan
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                      simState === 'IDLE' && currentAppTab === 'PROFILE' ? 'bg-emerald-500 text-slate-950 shadow' : 'bg-slate-800/80 text-slate-300 hover:text-white'
                    }`}
                  >
                    Profile
                  </button>
              ...
              {/* Phone Device Shell */}
              <div className="w-full max-w-[420px] h-[780px] bg-[#0b0e14] rounded-[48px] border-[7px] border-slate-800 shadow-2xl shadow-emerald-950/25 overflow-hidden relative flex flex-col select-none ring-1 ring-white/10">
                
                {/* Phone Top Notch Bar */}
                <div className="h-8 w-full flex items-center justify-between px-7 pt-2.5 z-30 bg-[#0b0e14]">
                ...
                <div className="flex-1 flex flex-col relative overflow-hidden bg-[#0a0d13]">
                  ...
                  {simState !== 'IDLE' ? (
                    <div className="flex-1 flex flex-col justify-between p-6 bg-gradient-to-b from-[#0c1017] via-[#090d14] to-[#0c1017] z-20">
                      ...
                      {simState === 'SEARCHING' && (
                        <div className="flex-1 flex flex-col items-center justify-center text-center">
                          <div className="relative flex items-center justify-center my-8">
                            <div className="absolute w-48 h-48 rounded-full border-2 border-emerald-400/20 animate-ping" style={{ animationDuration: '2s' }} />
                            <div className="absolute w-36 h-36 rounded-full border border-emerald-400/40 animate-pulse" />
                            <div className="w-28 h-28 rounded-full bg-slate-900 border-2 border-emerald-400 flex items-center justify-center shadow-lg shadow-emerald-500/20">
                              <Radio className="w-10 h-10 text-emerald-400 animate-spin" style={{ animationDuration: '8s' }} />
                            </div>
                          </div>

                          <span className="px-3.5 py-1.5 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold border border-emerald-500/20 mb-2">
                            Matching Level: {selectedEnglishLevel}
                          </span>
                          <h3 className="text-xl font-bold text-white">Searching for Partner...</h3>
                          <p className="text-sm text-slate-300 mt-1.5">Connecting with an active learner worldwide ({searchSecs}s)</p>

                          <button
                            onClick={handleCancelSearch}
                            className="mt-8 px-6 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm font-bold text-white transition-all cursor-pointer shadow"
                          >
                            Cancel Search
                          </button>
                        </div>
                      ...
                      {simState === 'CONNECTING' && (
                        <div className="flex-1 flex flex-col items-center justify-center text-center">
                          <div className="w-24 h-24 rounded-full bg-emerald-500/20 border-2 border-emerald-400 flex items-center justify-center mb-4">
                            <Zap className="w-11 h-11 text-emerald-400 animate-bounce" />
                          </div>
                          <h3 className="text-lg font-bold text-white">Connecting with {currentPartnerName}...</h3>
                          <p className="text-sm text-slate-300 mt-1.5">Establishing secure direct voice connection...</p>
                        </div>
                      )}
                      ...
                      {simState === 'IN_CALL' && (
                        <div className="flex-1 flex flex-col items-center justify-between py-2">
                          <div className="text-center mt-1 w-full flex flex-col items-center">
                            ...
                            {!isProUser ? (
                              <div className="px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-1.5 border transition-all bg-emerald-500/10 text-emerald-300 border-emerald-500/20 shadow-sm">
                                <span>⏱️ Free Call (Max 10 min) • {formatTimer(Math.max(0, 600 - callSecs))} left</span>
                              </div>
                            ) : (
                              <div className="px-4 py-2 rounded-full text-xs font-bold flex items-center gap-1.5 border bg-amber-500/10 text-amber-400 border-amber-500/30 shadow-sm">
                            ...
                            {!isProUser && (
                              <div className="flex items-center gap-2 mt-2 bg-slate-900/90 px-3 py-1.5 rounded-xl border border-slate-800 text-xs">
                                <span className="text-slate-400">Test Limit:</span>
                                <button
                                  onClick={() => handleFastForward(300)}
                                  className="px-2 py-1 rounded-lg bg-slate-800 text-slate-200 hover:text-white font-medium"
                                  title="Add 5 Minutes"
                                >
                                  +5m
                                </button>
                                <button
                                  ...
                                  className="px-2 py-1 rounded-lg bg-amber-500/20 text-amber-300 hover:bg-amber-500 hover:text-slate-950 font-bold"
                          ...
                          <div className="flex flex-col items-center my-auto">
                            <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-emerald-600 to-teal-500 border-4 border-slate-800 flex items-center justify-center shadow-xl shadow-emerald-950/40 text-3xl font-black text-slate-950">
                              {currentPartnerName.charAt(0)}
                            </div>
                            <h4 className="text-base font-bold text-white mt-3">{currentPartnerName}</h4>
                            <span className="text-sm text-slate-300">Speaker ID: #{currentPartnerId}</span>

                            {/* Audio Waveform Bars */}
                            <div className="flex items-center gap-2 h-10 mt-4">
                              {[18, 32, 48, 24, 40, 20, 36, 26].map((h, i) => (
                                <div
                                  key={i}
                                  className="w-2 bg-emerald-400 rounded-full transition-all duration-150 animate-pulse"
                                  style={{
                                    height: isMuted ? '5px' : `${h}px`,
                          ...
                          <div className="w-full flex items-center justify-around px-4 pb-7 pt-2">
                            ...
                            <button
                              ...
                              className="w-18 h-18 rounded-full bg-red-500 hover:bg-red-600 text-white shadow-xl shadow-red-500/40 flex items-center justify-center transition-transform active:scale-95 cursor-pointer"
                            >
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                          {isLimitReached ? (
                            <div className="w-full flex flex-col items-center space-y-3.5">
                              <div className="w-18 h-18 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 shadow-lg">
                                <Crown className="w-9 h-9" />
                              </div>
                              <h3 className="text-lg font-bold text-white">10-Minute Free Call Ended</h3>
                              <p className="text-sm text-slate-200 leading-relaxed max-w-[320px]">
                              ...
                              <div className="w-full space-y-2.5 pt-2">
                                <button
                                  ...
                                  className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-sm font-bold transition-all shadow"
                                >
                                ...
                                <button
                                  ...
                                  className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 border border-amber-500/40 text-amber-300 text-sm font-bold transition-all flex items-center justify-center gap-1.5"
                                >
                                  <Crown className="w-4 h-4" />
                            ...
                            <div>
                              <div className="w-18 h-18 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center mb-3 mx-auto">
                                <PhoneOff className="w-8 h-8 text-red-400" />
                              </div>
                              <h3 className="text-lg font-bold text-white">Call Ended</h3>
                              <p className="text-sm text-slate-300 mt-1">Duration: {formatTimer(callSecs)} • Free Session</p>
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                      ...
                      <div className="px-4 py-3 bg-[#0d1017] border-b border-slate-800/80 flex items-center justify-between z-10 shrink-0">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-xs font-bold text-slate-100 hover:border-emerald-500/50 transition-colors"
                          >
                            {profileImage ? (
                              <img src={profileImage} alt="" className="w-4 h-4 rounded-full object-cover" />
                            ) : (
                              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
                            )}
                            <span className="truncate max-w-[120px]">{isLoggedIn ? userName : 'Profile'}</span>
                          </button>
                          
                          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-amber-500/10 text-amber-400 text-xs font-bold border border-amber-500/20">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 text-xs font-bold shadow-sm hover:brightness-110"
                        >
                          <Crown className="w-4 h-4" />
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        {currentAppTab === 'HOME' && (
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3.5 p-3 rounded-2xl bg-gradient-to-r from-sky-950/40 via-slate-900/60 to-slate-900/40 border border-sky-500/20">
                              <img src="/logo.png" alt="SpeakFree" className="w-12 h-12 rounded-2xl shadow-md border border-sky-400/30 object-cover shrink-0" />
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-slate-300 truncate mt-0.5">Practice spoken English with real partners</p>
                              </div>
                            </div>

                            {/* Live Online Badge */}
                            <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-sm">
                            ...
                            <div className="p-6 rounded-3xl bg-gradient-to-b from-[#11151f] to-[#0d1017] border border-slate-800 text-center flex flex-col items-center shadow-lg">
                              <div className="relative flex items-center justify-center my-4">
                                <div className="absolute w-44 h-44 rounded-full bg-emerald-500/10 animate-ping" style={{ animationDuration: '3s' }} />
                                <div className="absolute w-34 h-34 rounded-full border border-emerald-500/30 animate-pulse" />
                                
                                <button
                                  onClick={handleStartSearch}
                                  className="w-28 h-28 rounded-full bg-gradient-to-b from-slate-800 to-slate-950 border-2 border-emerald-400 shadow-xl shadow-emerald-500/20 flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:scale-105 active:scale-95 transition-all group"
                                >
                                  <Mic className="w-8 h-8 text-emerald-400 group-hover:scale-110 transition-transform" />
                                  <span className="text-xs font-extrabold text-white">Find Partner</span>
                                </button>
                              </div>
                              ...
                              <div className="w-full mt-4 pt-3.5 border-t border-slate-800/80 flex items-center justify-between text-xs">
                                <span className="text-slate-300 font-medium">Your Level:</span>
                                <div className="flex items-center gap-1.5">
                            ...
                            <div className="space-y-2.5">
                              ...
                              <div className="grid grid-cols-1 gap-2.5">
                                <div className="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-bold text-white">💼 Job Interview Practice</h5>
                                    <p className="text-xs text-slate-300 mt-0.5">"Tell me about yourself & your strengths"</p>
                                  </div>
                                  <button
                                    onClick={handleStartSearch}
                                    className="px-3 py-1.5 rounded-xl bg-emerald-500/20 text-emerald-400 text-xs font-bold hover:bg-emerald-500 hover:text-slate-950 transition-all cursor-pointer"
                                  >
                                    Practice
                                  </button>
                                </div>
                                ...
                                <div className="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-bold text-white">✈️ Travel & Daily Routine</h5>
                                    <p className="text-xs text-slate-300 mt-0.5">"Favorite trip & airport conversations"</p>
                                  </div>
                                  <button
                                    onClick={handleStartSearch}
                                    className="px-3 py-1.5 rounded-xl bg-emerald-500/20 text-emerald-400 text-xs font-bold hover:bg-emerald-500 hover:text-slate-950 transition-all cursor-pointer"
                        ...
                        {currentAppTab === 'FRIENDS' && (
                          <div className="space-y-4">
                            {/* Add Friend Box */}
                            <form onSubmit={handleAddFriend} className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 flex gap-2">
                              <input
                                type="text"
                            ...
                            <div className="space-y-2.5">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs text-emerald-400 font-bold">2 Online Now</span>
                              </div>
                              ...
                              <div className="space-y-2.5">
                                {friendsList.map(friend => (
                                  <div
                                    key={friend.id}
                                    className="p-3.5 rounded-2xl bg-[#0f131a] border border-slate-800 flex items-center justify-between hover:border-slate-700 transition-colors"
                                  >
                                    <div className="flex items-center gap-3">
                                      <div className={`w-10 h-10 rounded-full ${friend.avatarColor} flex items-center justify-center text-sm font-bold text-white relative shadow-sm`}>
                                      ...
                                      <div>
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-slate-300 mt-0.5">
                                          <span className="text-slate-300">{friend.level}</span>
                                          <span className="text-slate-500">•</span>
                                          <span className="text-slate-400">{friend.location}</span>
                                        </div>
                                      </div>
                                    </div>
                        ...
                        {currentAppTab === 'SUBSCRIPTION' && (
                          <div className="space-y-4">
                            ...
                            {isProUser ? (
                              <div className="p-4 rounded-3xl bg-gradient-to-br from-amber-500/20 via-slate-900 to-[#1b1509] border border-amber-500/60 shadow-lg">
                                <div className="flex items-center justify-between">
                                ...
                                <div className="mt-3 text-center py-2.5 bg-slate-950/70 rounded-2xl border border-amber-500/20">
                                  <p className="text-xs text-slate-300 uppercase tracking-wider font-semibold">Subscription Valid Until</p>
                                  <p className="text-lg font-black text-white mt-1">{playExpiryDate || '05 Feb 2027'}</p>
                                  <p className="text-xs text-amber-400/90 font-mono mt-1">Order: {playOrderId}</p>
                                </div>
                              ...
                              <div className="p-5 rounded-3xl bg-gradient-to-br from-amber-500/20 via-slate-900 to-slate-950 border border-amber-500/40 text-center relative overflow-hidden shadow-lg">
                                ...
                                <button
                                  ...
                                  className="w-full mt-4 py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-lg bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 hover:brightness-110 cursor-pointer"
                                >
                            ...
                            <div className="space-y-2.5 p-4 rounded-2xl bg-slate-900/90 border border-slate-800">
                              <h4 className="text-xs font-bold text-slate-200 uppercase tracking-wider">What's included in VIP Pass:</h4>
                        ...
                        {currentAppTab === 'PROFILE' && (
                          <div className="space-y-4">
                            ...
                            {/* Profile Card */}
                            <div className="p-4 rounded-2xl bg-gradient-to-b from-[#111622] to-[#0c0f16] border border-slate-800 flex items-center justify-between">
                              <div className="flex items-center gap-3.5">
                                ...
                                >
                                  <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-400 p-0.5 shadow-md shadow-emerald-950/40 transition-transform group-hover:scale-105 active:scale-95">
                                    <div className="w-full h-full rounded-full bg-slate-900 overflow-hidden flex items-center justify-center font-bold text-base text-white">
                                      {profileImage ? (
                                        ...
                                        <div className="w-full h-full bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-slate-950 font-bold text-lg">
                                          {userName.charAt(0)}
                                        </div>
                                      )}
                                    </div>
                              ...
                              <button
                                ...
                                className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold text-emerald-400 border border-slate-700 self-start mt-1 cursor-pointer"
                              >
                            ...
                            <div className="grid grid-cols-3 gap-2.5 text-center">
                              <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800">
                                <span className="text-xs text-slate-400 block font-medium">Total Minutes</span>
                                <span className="text-base font-bold text-emerald-400 font-mono mt-0.5 block">145 min</span>
                              </div>
                              <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800">
                                <span className="text-xs text-slate-400 block font-medium">Calls Taken</span>
                                <span className="text-base font-bold text-white font-mono mt-0.5 block">28</span>
                              </div>
                              <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800">
                                <span className="text-xs text-slate-400 block font-medium">Streak</span>
                                <span className="text-base font-bold text-amber-400 font-mono mt-0.5 block">🔥 {userStreak}d</span>
                              </div>
                            </div>
                            ...
                            <div className="p-2.5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-1.5">
                              <button
                                onClick={() => setShowAuthModal(true)}
                                className="w-full px-3.5 py-3 rounded-xl flex items-center justify-between text-xs text-slate-200 hover:bg-slate-800 transition-colors font-medium cursor-pointer"
                              >
                                <div className="flex items-center gap-2.5">
                              ...
                              <div className="w-full px-3.5 py-3 rounded-xl flex items-center justify-between text-xs text-slate-200 font-medium">
                                <div className="flex items-center gap-2.5">
                      ...
                      <div className="h-16 bg-[#0d1017] border-t border-slate-800/90 px-3 flex items-center justify-around shrink-0 z-10">
                        <button
